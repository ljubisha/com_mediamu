...
class MediamuScript
{
	...
	private function _setParams( $params )
	{
                ...
                $this->runtimeScript            = $params->get('plupload_runtime', 'full');
                $this->runtime                  = $this->runtimeScript == 'full' ? $allRuntimes : $this->runtimeScript;
                
                //default 1MB
                $this->_maxFileSize             = $params->get('mediamu_max_file_size', 1);
                
                //chunk upload
		$this->_chunkSize 		= $params->get('chunk_size', 0);
                $this->_chunkUnit               = $params->get('chunk_unit', 'mb');
                $this->_chunkUnit               = strtolower($this->_chunkUnit);
                
                //file rename
		$this->_rename 			= ($params->get('rename') == 1) ? 'true' : 'false';
                
                //file filters
		$imageFilter                    = $params->get('image_file_extensions', 'jpg,png,gif');
                $this->_imageFilter             = $this->_cleanOption($imageFilter);
		$otherFilesFilter               = $params->get('other_files_extension', 'doc,txt');
                $this->_otherFilesFilter        = $this->_cleanOption($otherFilesFilter);
                
                //generate unique names for files
		$this->_uniqueNames		= $params->get('unique_names') == 1 ? 'true' : 'false';
                
                //image resizing
                $this->_resize 			= $params->get('enable_image_resizing', 0) == 0 ? false : true;
		$this->_resizeWidth 		= $params->get('resize_width', '640');
		$this->_resizeHeight		= $params->get('resize_height', '480');
		$this->_resizeQuality		= $params->get('resize_quality', '90');
	}
...
jQuery(document).ready(function($) {
	...
	function addLanguageSupport() {
            plupload.addI18n({
                'Select files' : '<?php echo JText::_('COM_MEDIAMU_UPLOADER_SELECT_FILES');  ?>',
                'Add files to the upload queue and click the start button.' : '<?php echo JText::_('COM_MEDIAMU_UPLOADER_ADD_FILES_TO_QUEUE')  ?>',
                'Filename' : '<?php echo JText::_('COM_MEDIAMU_UPLOADER_FILENAME')  ?>',
                'Status' : '<?php echo JText::_('COM_MEDIAMU_UPLOADER_STATUS')  ?>',
                'Size' : '<?php echo JText::_('COM_MEDIAMU_UPLOADER_SIZE')  ?>',
                'Add files' : '<?php echo JText::_('COM_MEDIAMU_UPLOADER_ADD_FILES')  ?>',
                'Start upload':'<?php echo JText::_('COM_MEDIAMU_UPLOADER_START_UPLOAD')  ?>',
                'Stop current upload' : '<?php echo JText::_('COM_MEDIAMU_UPLOADER_STOP_CURRENT_UPLOAD')  ?>',
                'Start uploading queue' : '<?php echo JText::_('COM_MEDIAMU_UPLOADER_START_UPLOADING_QUEUE') ?>',
                'Drag files here.' : '<?php echo JText::_('COM_MEDIAMU_UPLOADER_DRAG_FILES')  ?>'
            });
	}
			...
			init : {
				...
				StateChanged: function(up) {
					...
					if(up.state == plupload.STARTED) {
						...
						stopBtn.innerHTML = '<?php echo JText::_('COM_MEDIAMU_UPLOADER_STOP_UPLOAD')  ?>';
						stopBtn.href = '#',
					...
					if(up.state == plupload.STOPPED) {
						...
						refreshBtn.innerHTML = '<?php echo JText::_('COM_MEDIAMU_UPLOADER_REFRESH_UPLOADER')  ?>';
						refreshBtn.href = '#',
	...
	$('#delete_selected').click(function () {
            var cnfTxt = '<?php echo JText::_('COM_MEDIAMU_DIR_BROSWER_CONFIRM_DELETE_MULTIPLE'); ?>';
            if(confirm(cnfTxt)) {
